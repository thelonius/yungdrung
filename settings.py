...
`Задачи/`, эти данные не правят вручную в интерфейсе задач. Поэтому
файл лежит в корне стора, а не внутри `Задачи/`, и в остальном ничем не
...
стору settings.py вычисляет тем же способом независимо (переменная
...
# Тот же способ определить стор, что и в engine.py (переменная окружения,
...
def settings_path(vault=None):
    """Файл настроек — в корне стора, рядом с `Задачи/`, а не внутри неё:
...
def _reasons_archive(reasons, name):
    ...
    (CONTRACT.md), повтор после обрыва связи не должен пугать пользователя."""
