...
DEFAULTS = {
    ...
    # Пустой справочник — разумный дефолт для «настроек с нуля», а не потому
    # что причин быть не должно. Семь причин из engine.REASONS сюда намеренно
    # не скопированы: engine.py — источник истины на сегодня, копия здесь
    # разошлась бы с ним при первой же правке одного из двух списков. Перенос
    # стартового набора — разовый шаг при подключении модуля, см. отчёт.
    "reasons": [],
    "tags": [],
    "kb": {
...
def validate(data):
    ...
    # --- флаги -----------------------------------------------------------
    for field in ("autostart", "minimize_to_tray"):
        if field in data and not isinstance(data.get(field), bool):
            errors.append({"field": field, "error": "Флаг — да или нет"})
