...
пишет по-прежнему движок, иначе появится второй писатель стора.
...
арифметики над шаблонами у них нет. Переезд стора на SQLite означает третьего
...
def split_control_value(value):
    ...
    по-разному: из стора дату достаёт yaml уже разобранной, а из JSON и из
...
def same_name(one, other):
    ...
    сторе живут по тому же правилу, и различаться они не должны."""
...
def templates_path(vault):
    """Файл шаблонов лежит в сторе на виду, а не скрытым служебным: это данные
...
class JsonStore(Store):
    ...
    программа: свободного текста в нём нет. И запись «имя, теги, список шагов со сдвигом» ложится в
    таблицу SQLite один в один, поэтому переезд не потребует переразбора текста.
    Разобрать frontmatter было бы чем — движок читает им весь стор, — но
    ...
    def _commit(self, templates):
        """Атомарно: файл лежит в сторе, а стор синхронизируется OneDrive.
        Дописывание на месте даёт обрезанный файл, то есть
