...
`tmp_path`: у settings.py есть дефолтный путь рядом с настоящим стором
...
def test_архивирование_причины_идемпотентно(пустой):
    """Операции идемпотентны — правило из CONTRACT.md: повторный вызов после
...
# --- путь по умолчанию не совпадает с настоящим стором проекта -------------

def test_дефолтный_путь_не_настоящий_стор_проекта():
