...
def test_тело_заметки_не_трогаем(вольт):
    """Тело пишет заказчик: движок правит только frontmatter."""
...
def test_notdone_уважает_явную_дату(вольт):
    рез = задача(вольт, "Подшипник", [шаг(1, "Снять колесо", control_date=СЕГОДНЯ)])
