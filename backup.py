...
# заводит окружение прямо внутри вольта; гигабайты пакетов в копии не нужны и
# заказчику точно не принадлежат. Настройки Obsidian (.obsidian) наоборот берём
# — заказчик их настраивал руками, и после восстановления вольт должен
...
def diff_sqlite_tasks(old_db, new_db):
    ...
    Нужна `restore()` для базы SQLite: там вольт — один файл, и он подменяется
...
def restore(archive, vault, safety_dir=None, now=None):
    ...
    Для вольта-базы (SQLite) файл один, и он подменяется целиком — «лишнего
