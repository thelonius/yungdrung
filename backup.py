#!/usr/bin/env python3
"""Резервные копии стора и экспорт всей базы в JSON. Требования R25 и R11.

Две разные задачи, которые легко смешать.

**Копия (R25)** — снимок стора как он есть, чтобы вернуть данные после сбоя.
Один zip на копию: стор состоит из десятков файлов, и копировать их по одному
...
**Два вида хранилища.** Сейчас стор — папка с markdown, спринт 1 плана переводит
...
должен лишать заказчика возможности снять копию — это последняя защита данных.
  ...
  backup.backup("D:/Стор/стор.db", "D:/Копии", keep=7)     # после переезда
  backup.write_export(vault, "выгрузка.json")                # одной кнопкой
  backup.restore("D:/Копии/стор-2026-08-18-143005.zip", vault)
"""
import json
import os
import re
...
# Имя копии: «стор-2026-08-18-143005.zip». Метка времени в имени, а не только во
...
# Мусор сборки и служебные папки в копию не идут: .git в сторе заказчика может
# весить больше самих данных, а __pycache__ восстанавливать незачем. venv/.venv
# — по той же причине: на маке системный pip заблокирован PEP 668, и разработчик
# заводит окружение прямо внутри стора; гигабайты пакетов в копии не нужны и
# заказчику точно не принадлежат. Служебные каталоги редакторов наоборот берём
# — заказчик их настраивал руками, и после восстановления стор должен
# открыться таким же, а не голым.
...
SKIP_DIRS = {".git", "__pycache__", ".pytest_cache", ".ipynb_checkpoints",
            "venv", ".venv", attachments.DIR_NAME}
...
def parse_name(name, prefix=PREFIX):
    ...
    try:
        ...
        return None  # «стор-2026-13-45-999999.zip» — цифры на месте, даты нет
...
def is_sqlite(path):
    ...
    Имя файла базы выберет человек при переезде, и `.db`, `.sqlite`, `стор.дан`
...
def storage_kind(vault):
    """Чем стор является прямо сейчас: `folder`, `sqlite`, `file` или `missing`.
...
def collect(vault, skip_dirs=()):
    """Файлы стора: пары «путь, имя внутри архива». Имена в posix-виде.

    `skip_dirs` нужен для одного важного случая: папку копий заказчик вполне
    может указать внутри стора. Тогда каждая следующая копия включала бы в себя
...
def zip_entry(name, mtime):
    ...
    границ в сторе означает сбитые часы или кривой перенос файлов, и ронять
...
def create(vault, dest, now=None, prefix=PREFIX, raw_on_corrupt=False):
    """Копия стора в папку `dest`. Возвращает описание созданной копии.

    Стором может быть папка с файлами (сейчас) или один файл базы SQLite (после
    ...
    if вид == "missing":
        raise BackupError("vault", f"нет стора по пути: {vault}")
    dest = Path(dest)
    try:
    ...
    if вид == "folder":
        ...
        if not files:
            # Пустой стор означает, что путь указан не туда: диск не подключён,
...
def tags_of(meta):
    ...
    if isinstance(tags, str):
        tags = [tags]  # один тег строкой — нормализуем в список
...
def export(vault, now=None):
    ...
    Формат самодостаточный: по нему стор собирается заново без нашего кода.
    ...
    Чтобы собрать стор заново по этому файлу, чужой программе хватает
    ...
    идентификаторам, которые вне стора ничего не значат.

    Задачи и заметки — из разных хранилищ, и это видно только внутри функции, не
    в форме ответа. Задачи движок давно читает из `стор.db` (решение в
    CLAUDE.md, `_export_tasks`), заметки базы знаний markdown ещё не покинули —
    их по-прежнему собирает обход стора (`collect`) и разбор `База/*.md` тем же
    парсером YAML, что и раньше.

    Чего в выгрузке нет намеренно: вложений и любых нетекстовых файлов стора.
    ...
    vault = Path(vault)
    if not vault.is_dir():
        raise BackupError("vault", f"нет папки стора: {vault}")
    ...
    for путь, имя in collect(vault):
        ...
        if not raw.startswith("---"):
            continue  # обычный markdown рядом с стором, не запись трекера
        try:
...
def _export_tasks(vault):
    """Задачи для выгрузки — из `стор.db`, не сканированием `Задачи/*.md`.
    ...
    (`vault / "стор.db"`), а не через глобальную `engine.VAULT`: `export()`
    уже принимает свой путь параметром, и результат не должен зависеть от
    состояния другого модуля — иначе выгрузка стора, переданного явным путём,
    молча читала бы задачи не из него, а из того, что `engine.VAULT` укажет в
    этот момент.

    Файла `стор.db` может не быть вовсе — стор свежий или вообще без задач.
...
def safe_names(z):
    ...
    for info in z.infolist():
        ...
        имена.append(имя)
    if not имена:
        raise BackupError("archive", "в архиве нет файлов стора")
    return имена
...
def describe(archive):
    ...
    return {"file": str(archive), "name": archive.name,
            ...
            # куда копия разворачивается — в папку стора или в файл базы.
...
def target_entry(имена, vault):
    """Какая запись архива ложится в файл-стор.
    ...
    raise BackupError("archive",
                      f"в копии {len(имена)} файлов, а стор — один файл "
                      f"{vault.name}: укажите папку стора")


def diff_sqlite_tasks(old_db, new_db):
    """Задачи из `tasks` в `old_db`, которых нет в `new_db`, — по `id`.

    Нужна `restore()` для базы SQLite: там стор — один файл, и он подменяется
    целиком, а не файл за файлом, как для папки markdown. Поэтому «лишние»
    задачи (заведённые после копии) не новые файлы архива, а строки таблицы
    `tasks`, которых в снимке из архива ещё не было.

    Сравнение — по `id`, а не по заголовку: `id` растёт на каждую новую задачу
    и не меняется при переименовании, так что переименованная между копией и
    восстановлением задача не попадёт в список ошибочно — совпадёт по `id`,
    разойдётся только заголовком.

    Файл без таблицы `tasks` (заголовок SQLite на месте, а наша схема ещё не
    заведена или это вообще не наша база) — не повод падать: пустой список,
    сравнивать нечего.
    """
    def строки(путь):
        try:
            conn = connect_ro(путь)
        except sqlite3.Error:
            return {}
        try:
            return {r[0]: r[1] for r in conn.execute("SELECT id, title FROM tasks")}
        except sqlite3.Error:
            return {}
        finally:
            conn.close()

    старые, новые = строки(old_db), строки(new_db)
    return [старые[i] for i in sorted(старые) if i not in новые]


def restore(archive, vault, safety_dir=None, now=None):
    """Развернуть копию поверх стора. Возвращает, что восстановлено.
    ...
    Стор, как и при снятии копии, бывает папкой и одним файлом базы. Для файла
    берётся подходящая запись архива (`target_entry`), для папки — все записи.

    Файлы, которые есть в сторе, но которых нет в архиве, не удаляются, а
    ...
    Для стора-базы (SQLite) файл один, и он подменяется целиком — «лишнего
    файла» в этом случае не бывает, а задача, заведённая после копии, это не
    новый файл, а новая строка внутри той же базы. `extra` для этого случая
    несёт заголовки таких задач (см. `diff_sqlite_tasks`), а не имена файлов;
    они не удаляются восстановлением, но остаются только в страховочной копии
    (`safety_copy`) — молча вернуть их в базу восстановление не может.
    ...
    копии в поле `safety_copy`: стор в этот момент наполовину старый, и человек
    ...
    with z:
        ...
        if вид == "missing":
            # Пути нет вовсе: новая машина или потерянный стор. Чем он был,
        ...
        # файла базы — стор заменён заметкой, и ни одной ошибки по дороге.
        elif из_копии == "folder" and если_файл:
            raise BackupError("archive",
                              f"копия снята с папки стора, а {vault.name} — файл")
        elif из_копии in ("sqlite", "file") and not если_файл:
            raise BackupError("archive",
                              f"копия снята с файла стора, а {vault} — папка")
        if если_файл:
        ...
        # Пустой стор восстанавливают на новой машине или после потери папки:
        ...
        if есть_что_терять:
            # Стор под замену может быть уже битым — это самый частый повод
        ...
        try:
            ...
            # оборвётся на середине, стор к этому моменту ещё не тронут.
            for имя in имена:
                z.extract(имя, врем)
            if вид == "sqlite":
                # Сейчас, пока старая база ещё на месте, а новая уже распакована
                # рядом (но не легла поверх): дальше первая пропадёт целиком, и
                # сравнивать станет не с чем.
                лишние_задачи = diff_sqlite_tasks(vault, врем / имена[0])
            for имя in имена:
    ...
    из_архива = set(имена)
    if если_файл:
        лишние = лишние_задачи if вид == "sqlite" else []
    else:
        лишние = [имя for _, имя in collect(vault, skip_dirs=[куда_копию])
